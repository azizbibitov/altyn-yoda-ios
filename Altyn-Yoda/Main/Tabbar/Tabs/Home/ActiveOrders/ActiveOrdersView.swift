...
class ActiveOrdersView: UIView {
    ...
    func setupUI(){
        ...
        header.titleLabel.text = "active_orders".localized()
        ...
        noConnection.easy.layout([Edges(), Top().to(header, .bottom)])
        
        addSubview(noContent)
        noContent.easy.layout([Edges(), Top().to(header, .bottom)])
