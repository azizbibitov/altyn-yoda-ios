...
public struct ContextMenuItemWithImage: ContextMenuItem {
    ...
    public init(title: String, image: UIImage?, color: UIColor) {
        self.title = title
        self.image = image
        self.color = color
    }
...
public extension ContextMenuDelegate {
    func contextMenu(_ contextMenu: ContextMenu, targetedView: UIView, didSelect item: ContextMenuItem, forRowAt index: Int) -> Bool { return true }
    func contextMenuDidDeselect(_ contextMenu: ContextMenu, cell: ContextMenuCell, targetedView: UIView, didSelect item: ContextMenuItem, forRowAt index: Int) { }
    func contextMenuDidAppear(_ contextMenu: ContextMenu) { }
    func contextMenuDidDisappear(_ contextMenu: ContextMenu) { CM.MenuConstants.HorizontalMarginSpace = 20 }
}
...
public struct ContextMenuConstants {
    ...
    public var LabelDefaultFont : UIFont = UIFont(name: "Rubik", size: 14) ?? .systemFont(ofSize: 14)
    public var LabelDefaultColor : UIColor = UIColor.white.withAlphaComponent(0.95)
    public var ItemDefaultColor : UIColor = UIColor.black.withAlphaComponent(0.8)
    ...
    public var BackgroundViewColor : UIColor = .black.withAlphaComponent(0.8)
...
open class ContextMenu: NSObject {
    ...
    private var isShowingNow : Bool = false
    ...
    open func showMenu(viewTargeted: UIView, delegate: ContextMenuDelegate, animated: Bool = true, menuViewFrameOrigin: CGPoint? = nil ){
        if isShown { return } else { isShown = true }
        ...
        DispatchQueue.main.async {
            ...
            if menuViewFrameOrigin != nil {
                self.addTargetedImageView()
                self.openAllViews()
                self.targetedImageView.isHidden = true
                self.menuView.frame = CGRect (x: menuViewFrameOrigin!.x, y: menuViewFrameOrigin!.y, width: self.MenuConstants.MenuWidth, height: self.menuHeight)
            } else {
                self.addTargetedImageView()
                self.openAllViews()
            }
    ...
    func getRenderedImage(afterScreenUpdates: Bool = false) -> UIImage{
        ...
        let viewSnapShotImage = renderer.image { ctx in
            viewTargeted.contentScaleFactor = 1
    ...
    func addTargetedImageView(){
        ...
        targetedImageView.clipsToBounds = true
...
//        targetedImageView.layer.shadowOpacity = 0
    ...
    func openAllViews(animated: Bool = true, updateTagetedView: Bool = true ){
...
//            self.targetedImageView.layer.shadowOpacity = 0.2
    ...
    func prepareViewsForRemoveFromSuperView(with rect: CGPoint) {
        ...
        self.targetedImageView.isHidden = false
    ...
    func getZoomedTargetedSize() -> CGRect{
        ...
        return CGRect(x: updatedX - 20, y: updatedY, width: updatedWidth, height: updatedHeight)
